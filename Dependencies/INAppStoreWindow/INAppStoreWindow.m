...
- (void)mouseDragged:(NSEvent *)theEvent
{
    ...
    NSRect where = [window convertRectToScreen:NSMakeRect(theEvent.locationInWindow.x, theEvent.locationInWindow.y, 1, 1)];
    ...
    while ((theEvent = [NSApp nextEventMatchingMask:NSLeftMouseDownMask | NSLeftMouseDraggedMask | NSLeftMouseUpMask untilDate:[NSDate distantFuture] inMode:NSEventTrackingRunLoopMode dequeue:YES]) && (theEvent.type != NSLeftMouseUp)) {
        @autoreleasepool {
            ...
            deltaX += nowRect.origin.x - where.origin.x;
            deltaY += nowRect.origin.y - where.origin.y;
            ...
            where = nowRect; // this should be inside above if but doing that results in jittering while moving the window...
        }
    }
}
@end
...
- (void)setCloseButton:(INWindowButton *)closeButton
{
    if (_closeButton != closeButton) {
        ...
        if (_closeButton) {
            ...
            _closeButton.cell.accessibilitySubrole = NSAccessibilityCloseButtonSubrole;
            _closeButton.cell.accessibilityRoleDescription = NSAccessibilityRoleDescription(NSAccessibilityButtonRole, NSAccessibilityCloseButtonSubrole);
...
- (void)setMinimizeButton:(INWindowButton *)minimizeButton
{
    if (_minimizeButton != minimizeButton) {
        ...
        if (_minimizeButton) {
            ...
            _minimizeButton.cell.accessibilitySubrole = NSAccessibilityMinimizeButtonSubrole;
            _minimizeButton.cell.accessibilityRoleDescription = NSAccessibilityRoleDescription(NSAccessibilityButtonRole, NSAccessibilityMinimizeButtonSubrole);
...
- (void)setZoomButton:(INWindowButton *)zoomButton
{
    if (_zoomButton != zoomButton) {
        ...
        if (_zoomButton) {
            ...
            _zoomButton.cell.accessibilitySubrole = NSAccessibilityZoomButtonSubrole;
            _zoomButton.cell.accessibilityRoleDescription = NSAccessibilityRoleDescription(NSAccessibilityButtonRole, NSAccessibilityZoomButtonSubrole);
...
- (void)setFullScreenButton:(INWindowButton *)fullScreenButton
{
    if (_fullScreenButton != fullScreenButton) {
        ...
        if (_fullScreenButton) {
            ...
            _fullScreenButton.cell.accessibilitySubrole = NSAccessibilityFullScreenButtonSubrole;
            _fullScreenButton.cell.accessibilityRoleDescription = NSAccessibilityRoleDescription(NSAccessibilityButtonRole, NSAccessibilityFullScreenButtonSubrole);
...
- (void)_updateTitlebarView
{
    ...
    for (NSView *childView in _titleBarView.subviews) {
        if ([childView isKindOfClass:[NSControl class]]) {
            ((NSControl *)childView).enabled = isMainWindowAndActive;
        }
    }
}
