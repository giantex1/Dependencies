...
- (void)selectDefaultItem:(id)sender
{
...
{
	super.selectedItemIdentifier = itemIdentifier;
	...
	if ([item isKindOfClass:[SSSelectableToolbarItem class]])
	{
		...
		if (view && window)
		{
			...
			window.contentView = blankView;
			[window setFrame:winFrame display:YES animate:YES];
			window.contentView = view;
			
			// change title
			window.title = ssitem.label;
			...
			[[NSNotificationCenter defaultCenter] postNotificationName:SSST_SELECTION_CHANGED_NOTIFICATION object:nil];
		}
	}
}
...
-(void)selectItemWithIndex:(NSInteger)idx
{
	...
	for (NSToolbarItem* item in self.items)
	{
		if ([item isKindOfClass:[SSSelectableToolbarItem class]])
		{
			if (currIdx == idx)
			{
				self.selectedItemIdentifier = item.itemIdentifier;
				return;
			}
...
- (NSUInteger)indexOfSelectedItem
{
	...
	for (NSToolbarItem *item in self.items)
	{
		if ([item.itemIdentifier isEqualToString:self.selectedItemIdentifier])
		{
