...
@synthesize progressIndicator = progressIndicator_,
			keychainCheckbox = keychainCheckbox_,
            webView = webView_,
...
- (void)windowWillClose:(NSNotification *)note
{
	...
	// clear the cache in case we're re-opened
...
- (void)webView:(WebView *)sender didFinishLoadForFrame:(WebFrame *)frame
{
	...
	// TODO: not the smoothest experience but better than nothing at this point
	[self.webView stringByEvaluatingJavaScriptFromString:[NSString stringWithFormat:@"window.scrollTo(($(document).width()-$(window).width())/2,0)"]];
}
...
+ (BOOL)authorizeExternallyFromKeychainForService:(NSString *)service account:(NSString *)username label:(NSString *)label authentication:(GTMOAuth2Authentication *)newAuth
{
	[newAuth setAccessToken:nil];
	BOOL didGetTokens = NO;
	
	NSError *error = nil;
	NSString *password = [SAMKeychain passwordForService:service account:username label:label error:&error];
	if (password != nil)
	{
		[newAuth setKeysForResponseString:password];
		didGetTokens = YES;
...
#if DEBUG
	if (error)
		NSLog(@"Error fetching password for %@ account with username %@.\nError Description - %@", service, username, error.localizedDescription);
#endif
	return didGetTokens;
}
